...
void CopySections( const unsigned char *data, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module )
{
	...
	for ( i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++ )
	{
		...
		u_memcpy( dest, data + section->PointerToRawData, section->SizeOfRawData );
...
void FinalizeSections( PMEMORYMODULE module )
{
	...
	for ( i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++ )
	{
		...
		if ( size > 0 )
		{
			if ( _VirtualProtect((LPVOID)section->Misc.PhysicalAddress, section->SizeOfRawData, protect, &oldProtect) == 0 )
			{
				return;
			}
		}
...
int BuildImportTable(PMEMORYMODULE module)
{
	...
	for ( ; !_IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++ )
	{
		DWORD *thunkRef, *funcRef;
		HMODULE handle = (HMODULE)_LoadLibrary( (LPCSTR)(codeBase + importDesc->Name) );
		...
		{
			thunkRef = (DWORD *)(codeBase + (DWORD)importDesc->OriginalFirstThunk);
			funcRef = (DWORD *)(codeBase + (DWORD)importDesc->FirstThunk);
		} else 
		{
			thunkRef = (DWORD *)(codeBase + (DWORD)importDesc->FirstThunk);
			funcRef = (DWORD *)(codeBase + (DWORD)importDesc->FirstThunk);
		}
		for (; *thunkRef; thunkRef++, funcRef++)
		{
			...
			{
				*funcRef = (DWORD)_GetProcAddress( handle, (LPCSTR)IMAGE_ORDINAL(*thunkRef) );
			}
			else
			{
				PIMAGE_IMPORT_BY_NAME thunkData = (PIMAGE_IMPORT_BY_NAME)(codeBase + *thunkRef);
				DWORD Addr;
				if (  Addr = (DWORD)_GetProcAddress( handle, (LPCSTR)&thunkData->Name ))
				{
					if ( Addr != *funcRef)
						*funcRef = Addr;
				}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
	...
	code = (unsigned char *)_VirtualAlloc((LPVOID)(old_header->OptionalHeader.ImageBase),
		old_header->OptionalHeader.SizeOfImage,
		MEM_COMMIT,
		PAGE_READWRITE);

    if (code == NULL)
        code = (unsigned char *)_VirtualAlloc(NULL,
            old_header->OptionalHeader.SizeOfImage,
            MEM_COMMIT,
            PAGE_READWRITE);
	...
	result = (PMEMORYMODULE)u_alloc( sizeof(MEMORYMODULE), 1 );
	...
	u_memcpy(headers, (char*)dos_header, (DWORD)dos_header->e_lfanew + old_header->OptionalHeader.SizeOfHeaders);
